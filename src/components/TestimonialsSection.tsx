...
const TestimonialsSection = () => {
  ...
  const items = getJSON<typeof DEFAULT_TESTIMONIALS>("home.testimonials.items", DEFAULT_TESTIMONIALS);
  ...
  return (
    <section className="py-16 sm:py-20 md:py-28 bg-muted overflow-hidden relative">
      ...
      <div className="container mx-auto px-4 relative">
        <SectionHeading
          eyebrow={get("home.testimonials.eyebrow", "Testimonials")}
          title={get("home.testimonials.title", "What Our Clients Say")}
          description={get(
            "home.testimonials.description",
            "Trusted by fabricators, engineers, and project managers across the United States and Canada."
          )}
        />

        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8">
          {(items || DEFAULT_TESTIMONIALS).map((t, i) => (
            <motion.div
              key={`${t.name}-${i}`}
            ...
            >
              ...
              <div className="flex items-center gap-4 pt-5 border-t border-border">
                ...
                <div>
                  <h4 className="font-bold text-secondary font-heading text-base leading-none">{t.name}</h4>
                  <p className="text-xs text-primary mt-1.5 font-semibold uppercase tracking-wider">{t.role}</p>
